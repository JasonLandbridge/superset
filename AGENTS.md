...
> **⚠️ Personal Fork — Arch Linux Only**
> This is a custom fork of the upstream Superset project, released for **personal use on Arch Linux** only.
> It is not affiliated with, endorsed by, or representative of the official Superset project.
> All changes from upstream live on the `linux-fixes` branch.
...
## Linux Fork — Changes from Upstream

All changes live on the `linux-fixes` branch. Commits listed newest first.

### Build / Platform (Linux & Arch-specific)

- **Remove macOS and Windows from build** — Linux-only release.
- **`NODE_ENV` over `app.isPackaged`** — AUR Electron reports `isPackaged=true` even in dev mode, breaking dev/prod detection across host service, terminal daemon, and dev output piping.
- **Isolate dev home directory** — Append `-dev` to the Superset home directory in development to prevent daemon socket and SQLite conflicts with the production build.
- **Separate dev identity** — Use a distinct app name and `userData` directory in dev so it doesn't clash on the single-instance lock.
- **Prioritize dev migration path** — Dev migration directory checked before `isPackaged` fallback, since AUR Electron reports as packaged.
- **CI: `workflow_dispatch` trigger** — Manual trigger with secret fallbacks for testing local builds.
- **CI: `write-all` permissions** — Workaround for HTTP 403 when creating releases from a forked repository.
- **`RELAY_URL` fallback** — Prevents `undefined` literal at build time when env var is not set.

### Bug Fixes

- **PermissionRequest for `ask_user_question`** — Pi's tool calls now fire OS-level permission notifications through the existing `notify.sh` pipeline, matching Claude/Codex behavior.
- **Linux terminal shortcuts** — Fixed keyboard shortcuts for terminal panes on Linux such that Ctrl + c and ctrl + v work as copy and paste.
- **Reactive terminal tab names** — Removed `titleOverride` from new tab creation so live OSC terminal titles can drive tab names instead of being stuck on the launch label.
